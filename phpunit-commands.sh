...
cd ~/monkblog-php/ && mv .env .env.bak && mv .env.testing .env && php artisan migrate && php artisan db:seed && php -d memory_limit=-1 vendor/bin/phpunit && mv .env .env.testing && mv .env.bak .env